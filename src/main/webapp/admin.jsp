	...
	<script type="text/javascript">
        window.onload=function(){
            ...
            changeUser = function(operate , flag){
                switch (operate) {
                    case "remove":window.location="${pageContext.request.contextPath}/delete.do?id="+id;break;
                    case "changeStatu":window.location="${pageContext.request.contextPath}/zhuangtai.do?id="+id+"&status="+status;break;
                    case "findu": document.getElementById("form1").submit();break;
                    default: id=operate; status=flag;
                }
                console.log(id);
                console.log(status);
            };
        ...
        function  test(){
            var  count=$("#c").val();
            location.href="${pageContext.request.contextPath}/showAll.do?number="+count;
        }
				...
				<div class="collapse navbar-collapse"
					...
					<ul class="nav navbar-nav navbar-right">
						<li class="dropdown"><a href="#" class="dropdown-toggle"
												data-toggle="dropdown" role="button" aria-expanded="false">管理员[${sessionScope.name}]
							<span class="caret"></span>
						</a>
							<ul class="dropdown-menu" role="menu">
								<li><a href="#">个人资料</a></li>
								<li class="divider"></li>
								<li><a href="${pageContext.request.contextPath}/tuichu.do">退出</a></li>
							</ul></li>
					</ul>
				</div>
				<!-- /.navbar-collapse -->
			...
			<div class="table-responsive">
				<div class="row">
					...
					<div class="col-md-3 form-group ">
						<form class="form-inline" action="${pageContext.request.contextPath}/sousuo.do" method="post" id="form1">
				...
				<div class="form-group col-md-12 collapse" id="addUserForm">
					<div class="well">
						<form action="${pageContext.request.contextPath}/tianjia.do" method="post">
				...
				<div class="form-group col-md-12 collapse" id="updateUserForm">
					<div class="well">
						...
						<table class="table-condensed " style="text-align: center;">
							...
							<form id="ff"   class="" action="${pageContext.request.contextPath}/update.do" method="post" >
								<tbody>
								<tr id="updateTr">
									<td><input required="required" id="id"
											   class="form-control" type="text"
											   aria-describedby="inputGroupSuccess1Status"
											   readonly="readonly"  name="id"/>
									</td>
									<td><input required="required" id="name"
											   class="form-control" type="text"
											   aria-describedby="inputGroupSuccess1Status"
				...
				<table  style="text-align:center" id="userPager"
					...
					<tr class="bg-danger" >
						...
						<td>${list.ip}</td>
						<td>
							<c:if test="${list.status eq '正常'}">
							<a type="button" data-toggle="modal" data-target="#unFreezUser" class="btn btn-danger btn-sm" onclick="changeUser('${list.id}', '冻结')">正常</a>
							</c:if>
							<c:if test="${list.status eq '冻结'}">
								<a type="button" data-toggle="modal" data-target="#freezUser" class="btn btn-danger btn-sm" onclick="changeUser('${list.id}', '正常')">冻结</a>
							</c:if>
						</td>
						<td>
							<div class="btn-group" role="group">
								<button class="btn btn-default btn-sm" data-toggle="modal"
										data-target="#deleteConfirm" onclick="changeUser('${list.id}');">
									<span class="glyphicon glyphicon-remove-sign"></span> 删除
								</button>

								<button class="btn btn-default btn-sm" type="button"
										data-toggle="collapse"  data-target="#updateUserForm"  onclick="selectUser('${list.id}')">
					...
					<div >
						<ul class="pagination">
							...
							<li><a href="javascript:void(0)">
								<select id="c"  onchange="test()" name="number" >
									<option value="0">选择条数</option>
									<option value="3" <c:if test="${requestScope.number eq 3}">selected</c:if>>3</option>
									<option value="5" <c:if test="${requestScope.number eq 5}">selected</c:if>>5</option>
									<option value="10" <c:if test="${requestScope.number eq 10}">selected</c:if>>10</option>
								</select>
							</a>

							</li>
							<li><a href="#">条</a></li>
							<li><a href="#">共${requestScope.zongyeshu}页</a></li>
							<li <c:if test="${requestScope.yeshu eq 1}">class="disabled"</c:if>>
								<a href="${pageContext.request.contextPath}/showAll.do?yeshu=${requestScope.yeshu-1}&number=${requestScope.number}"> 上一页</a>
							</li>
							<li <c:if test="${requestScope.yeshu eq requestScope.zongyeshu}">class="disabled"</c:if>>
								<a href="${pageContext.request.contextPath}/showAll.do?yeshu=${requestScope.yeshu+1}&number=${requestScope.number}"   > 下一页</a>
							</li>
						</ul>
					</div>
				</div>
